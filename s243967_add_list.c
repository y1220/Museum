...
museum_res *reservation_by_museum(museum_res *,char *, int , int , museum *, int ,int ,int *);
...
int main(int argc, char *argv[]){
    FILE *fp;
    char tmp[N], tmp_1[N], tmp_2[N], filename[N];
    int cnt_t, cnt_m=0, i;
    if(argc==1){
    ...
    char name[N], m_name[N], x[N],y[N],z[N],w[N];
    float s_t, f_t;     //position of time slot, museum
    int day, n_peo, req, n_ti, n_mu, flag=1, num=1, repeat=1, succeed;
    ...
    if(req==1){
                ...
                scanf("%s %s %s %s %s %s",m_name, x, y, z, w, name);///to avoid having problem if user insert string into the position of int or float
                day=atoi(x);
                s_t=atof(y);
                f_t=atof(z);
                n_peo=atoi(w);
                if((strcmp(x,"0")!=0)&&(day==0)){///in the case day=0 it's acceptable, if not ERROR
                    printf("ERROR!(DAY must be 0 to 364)\n");
                    continue;
                }
                if(n_peo==0){///in the case n_peo=0 it's unacceptable
                    printf("ERROR!(Number of people has to be integer number)\n");
                    continue;
                }
                repeat=1;
        ...
        if(n_ti>=0){///5.make a reservation
            num++;
            head_m=reservation_by_museum(head_m, m_name, day, n_peo, muse, n_ti, n_mu, &succeed);
            if(succeed){
                num--;
                continue;
            }
    ...
    if(tmp5==NULL){
        printf("NULL\n");
        exit(EXIT_FAILURE);
    }else if((tmp5->next_nord==NULL)&&(tmp5!=NULL)){
        printf("NO.1: %s\n",tmp5->name);
    }
    while(tmp5->next_nord!=NULL){
        if(tmp5->next_diff_p!=NULL){
            while(tmp5->next_diff_p!=NULL){
                a++;
                printf("NO.%d: %s\n",a,tmp5->name);
                tmp5=tmp5->next_diff_p;
            }
            a++;
            printf("NO.%d: %s\n",a,tmp5->name);
            break;
...
museum_res *new_nord_m(museum *muse, museum_res *tmp, int i){
    ...
    for(j=0;j<muse[i].n_time;j++){
        ...
        printf("NEW NORD muse[%d].max_people[%d]: %d\n", i, j,muse[i].max_people[j]);
    }
    return tmp;
}
...
museum_res *reservation_by_museum(museum_res *head_m,char *m_name, int day, int n_peo, museum *muse, int n_ti, int n_mu, int *succeed){
    museum_res *tmp=head_m;
    head_m=tmp;
    int i=0, x, p_succeed=0;///0=succeed, 1=failed
    while(strcmp(tmp->name_m_res,m_name)!=0){
        ...
        if(tmp->day_res==-1){///INITIAL
            ...
            if(x<0){
                ...
                *succeed=p_succeed;
                printf("ERROR!(TOO MUCH PEOPLE)\n");
                printf("x=tmp->max_people_res[n_ti]-n_peo\n");
                printf("1.%d=%d-%d\n", x, tmp->max_people_res[n_ti], n_peo);
                return head_m;
            }
            tmp->day_res=day;
        ...
        tmp->next_day=tmp2;
        *succeed=p_succeed;
        x=tmp2->max_people_res[n_ti]-n_peo;
        printf("x=tmp->max_people_res[n_ti]-n_peo\n");
        printf("7.%d=%d-%d\n", x, tmp2->max_people_res[n_ti], n_peo);
...
reserve *connect_name_list(reserve *head,char *name){
    ...
    while(tmp3->next_diff_p==NULL){
        if(tmp3->next_nord==NULL){///initial
            tmp->next_diff_p=tmp3;
            return head;
        }
        tmp3=tmp3->next_nord;
    }
...
int check_visitor(reserve *head, char *name){
    ...
    while(tmp->next_nord!=NULL){
            while(tmp->next_diff_p==NULL){
                tmp=tmp->next_nord;
            }if(tmp->next_diff_p!=NULL){
